$(function(){
	...
	$.ajax({
		type:"post",
		url:"getAllExamineItem",
		data:{},
		dataType:"json",
		success:function(msg){
			console.log(msg);
			for(var i=0;i<msg.length;i++){
				$("#examineitem").append("<option value='"+msg[i].examine_id+"'data-price='"+msg[i].examine_price+"'>"+msg[i].examine_name+"</option>")
			}
		}
	});
	...
	var count=0;
	//选择项目，加入检查项目列表
	$("#examineitem").change(function(){
		var option=$("#examineitem option:selected");
		...
		//表格追加tr
		if(flag==false){
			$("#item_data").append("<tr><td>"+option[0].text+"</td><td><input type='hidden' name='elist["+count+"].examine_id' value='"+option[0].value+"'><input type='text' name='elist["+count+"].examine_amount' class='amount' value='1'></td><td>"+option.attr("data-price")+"</td><td><img class='trash' src='img/trash_13.1328px_1217857_easyicon.net.png'</td></tr>")
		}
	...
	$("#submitItem").click(function(){
		$.ajax({
			type:"post",
			url:"saveExamineItemRecord/"+$("#pid").val()+"/"+$("#doctor_id").val(),
